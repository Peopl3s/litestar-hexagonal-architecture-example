...
@pytest.fixture(scope="session") # Параметр scope="session" означает, что событийный цикл будет создан один раз на всю сессию тестирования, а не для каждого теста отдельно
...
async def db_engine() -> AsyncGenerator[AsyncEngine, None]:
    ...
    async with engine.begin() as conn:
        await conn.run_sync(base.UUIDAuditBase.metadata.create_all) #Создает все таблицы в базе данных с помощью base.UUIDAuditBase.metadata.create_all.
    yield engine # Выдает движок для использования в тестах.
    async with engine.begin() as conn:
        await conn.run_sync(base.UUIDAuditBase.metadata.drop_all) # После завершения тестов удаляет все таблицы (base.UUIDAuditBase.metadata.drop_all)
